...
var CompleteUserAuth = func(res http.ResponseWriter, req *http.Request) (goth.User, error) {
	...
	// use this access token for soql request
...
func validateState(req *http.Request, sess goth.Session) error {
	...
	fmt.Println("------")
	fmt.Println(originalState)
	fmt.Println(reqState)

	if originalState != "0" && (originalState != reqState) {
		return errors.New("state token mismatch")
	}
	authURL.Query().Set("state", "0")
	return nil
}
...
func GetFromSession(key string, req *http.Request) (string, error) {
	...
	if err != nil {
		return "", errors.New("could not find a matching session for this request GetFromSession")
	}
...
func getSessionValue(session *sessions.Session, key string) (string, error) {
	...
	if value == nil {
		return "", fmt.Errorf("could not find a matching session for this request getSessionValue")
	}
