...
import (
	...
	"SalesforceGit/cdcSubscribe/authentication"
	"SalesforceGit/cdcSubscribe/common"
	"SalesforceGit/cdcSubscribe/oauth"
...
func (c *PubSubClient) Authenticate() error {
	...
	sfRequest := authentication.AuthRequest{
		URL:      "https://login.salesforce.com",
		Username: string(subscribeValues.Username),
	}
	// disconnect from db

	authResponse, err := authentication.Authenticate(sfRequest, http.DefaultClient)
	...
	c.accessToken = authResponse.GetToken()
	c.instanceURL = authResponse.GetInstanceURL()
	//common.AuthToken = c.accessToken
...
// Subscribe Wrapper function around the Subscribe RPC. This will add the OAuth credentials and create a separate streaming client that will be used to
...
func (c *PubSubClient) Subscribe(replayPreset proto.ReplayPreset, replayId []byte) ([]byte, error) {
	...
	for {
		...
		for _, event := range resp.Events {
			...
			err = json.Unmarshal([]byte(codec.Schema()), &schema)
			if err != nil {
				fmt.Println("Error parsing Avro schema:", err)
			}
			fmt.Println("codec.Schema()")

			db.UpdateSchema(c.Topic.ChannelName, codec.Schema())
			...
			err = db.SaveEventToDB(c.Topic.ChannelName, body)
			if err != nil {
				fmt.Println(err)
			}
			if !ok {
...
func (c *PubSubClient) Publish(schema *proto.SchemaInfo) error {
	...
	if result == nil {
		return fmt.Errorf("nil result returned when publishing to %s", c.Topic.ChannelName)
	}
